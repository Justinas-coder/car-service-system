...
{{--                            x-on:click="document.querySelector('#delete-order-{{ $order->id }}').submit()"--}}
                        >
                            Delete
                        </button>
                        <button
    ...
    <div class="py-8 mx-auto w-full sm:w-2/3">
        <form class="max-w-7xl mx-auto sm:px-6 lg:px-8 flex"
              x-data="orderForm" method="POST" action="{{ route('orders.store') }}">
            @csrf
                    ...
                    @if(Gate::allows('isAdmin'))
                        ...
                        <td class="relative whitespace-nowrap py-4 pr-4 text-center text-sm font-medium sm:pr-3">
                            <form
                                ...
                                x-data=""
                                @submit.prevent="$dispatch('open-modal')">
                                <button x-data=""
                                        x-on:click="$dispatch('open-modal', 'delete-order-{{ $order->id }}');
